...
def setup_vina(software, source_file, adfr_tarball):
    ...
    print('Finished! To check if everything worked, run "pyscreener check vina METADATA" next!')
...
def extract_adfr_tarball(adfr_tarball):
    ...
    install_destination = input(
        f"Destination under which to install ADFR suite (default = {DEFAULT}): "
    ) or DEFAULT
...
def setup_dock6(source_tarball, chimera_installer):
    if "DOCK6" in os.environ:
        print("DOCK6 environment variable detected! Checking for proper configuration ...")
    ...
    print('Finished! To check if everything worked, run "pyscreener check dock6 METADATA" next!')
...
def install_sphgen_cpp():
    SPHGEN_URL = "http://dock.compbio.ucsf.edu/Contributed_Code/code/sphgen_cpp.1.2.tar.gz"
    try:
        resp = requests.get(SPHGEN_URL)

        DEFAULT = Path.home()
        download_dir = Path(
            input(f"Download destination for sphgen_cpp (default = {DEFAULT}): ") or DEFAULT
        )
